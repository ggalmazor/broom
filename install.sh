...
if [ "$VERSION" = "latest" ]; then
  ...
  VERSION=$(curl -fsSL https://api.github.com/repos/ggalmazor/broom/releases/latest | grep '"tag_name"' | sed -E 's/.*"v?([^"]+)".*/\1/')
  if [ -z "$VERSION" ]; then
...
URL="https://github.com/ggalmazor/broom/releases/download/$VERSION/$FILE"
